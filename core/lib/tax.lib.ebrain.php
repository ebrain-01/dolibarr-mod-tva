...
function tax_by_thirdparty($type, $db, $y, $date_start, $date_end, $modetax, $direction, $m = 0, $q = 0)
{
    ...
    if ($direction == 'sell')
    {
    	...
    	$invoicefieldref = 'ref';
		$categorythird = 'categorie_societe'; // ebrain
    }
    elseif ($direction == 'buy')
    {
    	...
    	$invoicefieldref = 'ref';
		$categorythird = 'categorie_fournisseur'; //ebrain
    }
    	...
    	|| ($direction == 'buy' && $conf->global->TAX_MODE_BUY_PRODUCT == 'invoice'))
    {
		...
		$sql .= " d.vat_src_code as vat_src_code, c.label as category_label, lc.fk_category as lc_third, "; //ebrain
		...
		$sql .= " ".MAIN_DB_PREFIX.$categorythird." as lc"; //ebrain
		$sql .= " ".MAIN_DB_PREFIX."category as c";//ebrain
		...
		$sql .= " AND s.rowid = lc.fk_soc"; //ebrain
		$sql .= " AND c.rowid = lc.fk_categorie"; //ebrain
    ...
    if ($sql != 'TODO')
    {
    	...
    	if ($resql)
    	{
    		...
    		while ($assoc = $db->fetch_array($resql))
    		{
    			...
    			$list[$assoc['company_id']]['drate'][] = $assoc['tri'];
...
function tax_by_rate($type, $db, $y, $q, $date_start, $date_end, $modetax, $direction, $m = 0)
{
    	...
    	|| ($direction == 'buy' && $conf->global->TAX_MODE_BUY_PRODUCT == 'invoice'))
    {
		...
		$sql .= " d.vat_src_code as vat_src_code, c.label as category_label, lc.fk_categorie as lc_third, "; //ebrain
		...
		$sql .= " ".MAIN_DB_PREFIX.$categorythird." as lc,"; //ebrain
		$sql .= " ".MAIN_DB_PREFIX."categorie as c,";//ebrain
    ...
    if ($sql != 'TODO')
    {
        ...
        if ($resql)
        {
            ...
            {
				// regroupement ebrain
				$assoc['tri'] = $assoc['vat_src_code'].'-'.$assoc['rate'];
            	// Code to avoid warnings when array entry not defined
            	if (!isset($list[$assoc['tri']]['totalht']))   $list[$assoc['tri']]['totalht'] = 0;
                if (!isset($list[$assoc['tri']]['vat']))       $list[$assoc['tri']]['vat'] = 0;
                if (!isset($list[$assoc['tri']]['localtax1'])) $list[$assoc['tri']]['localtax1'] = 0;
                if (!isset($list[$assoc['tri']]['localtax2'])) $list[$assoc['tri']]['localtax2'] = 0;

                if ($assoc['rowid'] != $oldrowid)       // Si rupture sur d.rowid
                {
                    $oldrowid = $assoc['rowid'];
                    $list[$assoc['tri']]['totalht']   += $assoc['total_ht'];
                    $list[$assoc['tri']]['vat']       += $assoc['total_vat'];
                    $list[$assoc['tri']]['localtax1'] += $assoc['total_localtax1'];
                    $list[$assoc['tri']]['localtax2'] += $assoc['total_localtax2'];
                }
				
				$list[$assoc['tri']]['rate'] = $assoc['rate'];//ebrain
				$list[$assoc['tri']]['vat_src_code'] = $assoc['vat_src_code'];//ebrain
				$list[$assoc['tri']]['category_label'] = $assoc['category_label'];//ebrain
				
                $list[$assoc['tri']]['dtotal_ttc'][] = $assoc['total_ttc'];
                $list[$assoc['tri']]['dtype'][] = $assoc['dtype'];
                $list[$assoc['tri']]['datef'][] = $db->jdate($assoc['datef']);
                $list[$assoc['tri']]['datep'][] = $db->jdate($assoc['datep']);
                $list[$assoc['tri']]['company_name'][] = $assoc['company_name'];
                $list[$assoc['tri']]['company_id'][] = $assoc['company_id'];
                $list[$assoc['tri']]['ddate_start'][] = $db->jdate($assoc['date_start']);
                $list[$assoc['tri']]['ddate_end'][] = $db->jdate($assoc['date_end']);

                $list[$assoc['tri']]['facid'][] = $assoc['facid'];
                $list[$assoc['tri']]['facnum'][] = $assoc['facnum'];
                $list[$assoc['tri']]['type'][] = $assoc['type'];
                $list[$assoc['tri']]['ftotal_ttc'][] = $assoc['ftotal_ttc'];
                $list[$assoc['tri']]['descr'][] = $assoc['descr'];

                $list[$assoc['tri']]['totalht_list'][] = $assoc['total_ht'];
                $list[$assoc['tri']]['vat_list'][] = $assoc['total_vat'];
                $list[$assoc['tri']]['localtax1_list'][] = $assoc['total_localtax1'];
                $list[$assoc['tri']]['localtax2_list'][] = $assoc['total_localtax2'];

                $list[$assoc['tri']]['pid'][] = $assoc['pid'];
                $list[$assoc['tri']]['pref'][] = $assoc['pref'];
                $list[$assoc['tri']]['ptype'][] = $assoc['ptype'];

                $list[$assoc['tri']]['payment_id'][] = $assoc['payment_id'];
                $list[$assoc['tri']]['payment_amount'][] = $assoc['payment_amount'];
		...
		if ($sql != 'TODO')
		{
			...
			if ($resql)
			{
				...
				while ($assoc = $db->fetch_array($resql))
				{
					...
					$list[$assoc['tri']]['dtotal_ttc'][] = $assoc['total_ttc'];
					$list[$assoc['tri']]['dtype'][] = 'ExpenseReportPayment';
					$list[$assoc['tri']]['datef'][] = $assoc['datef'];
					$list[$assoc['tri']]['company_name'][] = '';
					$list[$assoc['tri']]['company_id'][] = '';
					$list[$assoc['tri']]['user_id'][] = $assoc['fk_user_author'];
					$list[$assoc['tri']]['ddate_start'][] = $db->jdate($assoc['date_start']);
					$list[$assoc['tri']]['ddate_end'][] = $db->jdate($assoc['date_end']);
					...
					$list[$assoc['tri']]['pid'][] = $assoc['pid'];
					$list[$assoc['tri']]['pref'][] = $assoc['pref'];
					$list[$assoc['tri']]['ptype'][] = 'ExpenseReportPayment';
